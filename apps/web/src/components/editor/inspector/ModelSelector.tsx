      ...
      {favoriteModels.length > 0 && (
        <div className="space-y-1.5">
          <span className="text-[9px] text-text-muted flex items-center gap-1">
            <Star size={9} className="text-amber-400 fill-amber-400" /> {t("inspector.favoriteModels")}
          </span>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <span className="truncate">
            {isLoadingModels ? t("inspector.loadingModels") : getSelectedModelName()}
          </span>
      ...
      {showAllModels && (
        <div className="border border-border rounded-lg overflow-hidden">
          <div className="max-h-48 overflow-y-auto">
            {allModels.length === 0 ? (
              <div className="p-3 text-center text-[10px] text-text-muted">
                {isLoadingModels ? t("inspector.loadingModels") : t("inspector.noModelsAvailable")}
              </div>
            ) : (
              allModels.map((model) => {
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      title={isFav ? t("inspector.removeFromFavorites") : t("inspector.addToFavorites")}
                    >
                      {isFav ? (
          ...
          <div className="px-2 py-1 border-t border-border bg-background-secondary text-[8px] text-text-muted text-center">
            {allModels.length} {t("inspector.modelsAvailable")}
          </div>
        </div>
      )}
    </div>

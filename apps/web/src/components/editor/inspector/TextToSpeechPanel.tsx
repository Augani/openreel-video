...
export const TextToSpeechPanel: React.FC = () => {
  ...
  const warnUnsavedAudio = useCallback(() => {
    if (hasUnsavedAudio) {
      toast.warning(t("tts.unsavedDiscarded"), t("tts.unsavedDiscardedDesc"));
    }
  }, [hasUnsavedAudio, t]);
  ...
  return (
    <div className="space-y-3 w-full min-w-0 max-w-full">
      ...
      <div className="flex items-center justify-between p-2 bg-primary/10 rounded-lg border border-primary/30">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[9px] text-text-muted">{t("tts.aiVoiceGeneration")}</p>
          </div>
        </div>
        <button
      ...
      {provider === "piper" && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <label className="text-[10px] font-medium text-text-secondary">{t("tts.speed")}</label>
      ...
      <p className="text-[9px] text-text-muted text-center">
        {t("tts.poweredBy", {
          provider: provider === "elevenlabs" ? "ElevenLabs" : "Piper TTS",
          model: provider === "elevenlabs" ? ` · ${getSelectedModelName()}` : "",
        })}
      </p>
    </div>
  );

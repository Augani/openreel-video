...
export const AutoCaptionPanel: React.FC = () => {
  ...
  const handleStartTranscription = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : t("caption.failedTranscription"),
      );
      setIsTranscribing(false);
    }
  }, [getSpeechToTextEngine, selectedLanguage, t]);
    ...
    return (
      <div className="p-4 space-y-3">
        <div className="flex items-center gap-2 text-status-warning">
          <AlertCircle size={16} />
          <span className="text-[11px] font-medium">{t("caption.browserNotSupported")}</span>
        </div>
        <p className="text-[10px] text-text-muted">
          {t("caption.browserNotSupportedDesc")}
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4 w-full min-w-0 max-w-full">
      ...
      <div className="space-y-3 p-3 bg-background-tertiary rounded-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-text-secondary">{t("caption.language")}</span>
          </div>
          <Select
        ...
        <div className="flex items-center justify-between">
          <span className="text-[10px] text-text-secondary">{t("caption.captionStyle")}</span>
          <Select
            value={selectedStyle}
      ...
      {isTranscribing && progress && (
        <div className="space-y-2 p-3 bg-background-tertiary rounded-lg">
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-text-secondary">{t("caption.status")}</span>
            <div className="flex items-center gap-1">
              <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse" />
              <span className="text-[10px] text-red-400">{t("caption.recording")}</span>
            </div>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-text-secondary">
              {t("caption.segmentsFound")}
            </span>
      ...
      {segments.length > 0 && !isTranscribing && (
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-[10px] text-text-secondary">
              {t("caption.detected", { count: segments.length })}
            </span>
            <button
      ...
      <div className="flex gap-2">
        {!isTranscribing ? (
          ...
          >
            <Mic size={16} />
            <span className="text-[11px] font-medium">{t("caption.startRecording")}</span>
          </button>
        ) : (
          <button
          ...
          >
            <MicOff size={16} />
            <span className="text-[11px] font-medium">{t("caption.stopRecording")}</span>
          </button>
        )}
      </div>

      <p className="text-[9px] text-text-muted text-center">
        {t("caption.speakClearly")}
      </p>
    </div>
  );
};
